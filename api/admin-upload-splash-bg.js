...
// Adds or removes one of the splash screen's background images. Unlike the
// login background (a single app_settings column), the splash can hold many -
// the app picks one at random on every launch - so this is add/delete against
// the splash_backgrounds table rather than upsert/clear against one row.
...
export default async function handler(req, res) {
  ...
  if (action === 'delete') {
    ...
    const { error: deleteErr } = await supabase.from('splash_backgrounds').delete().eq('id', id);
  ...
  try {
    ...
    const path = `splash/${randomUUID()}.${ext}`;
